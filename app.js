...
const reactionTypes = [
  {key:'legendary',emoji:'🤘',label:'Legendär'},
  {key:'banger',emoji:'🔥',label:'Banger'},
  {key:'brutal',emoji:'☠️',label:'Brutal'},
  {key:'atmosphere',emoji:'🌑',label:'Atmosphäre'},
  {key:'riff',emoji:'🎸',label:'Riff des Todes'},
  {key:'goosebumps',emoji:'🖤',label:'Gänsehaut'}
];

function hideLoader(){
  const loader = document.getElementById('loader');
  if(loader) loader.classList.add('hide');
  document.documentElement.classList.add('loader-failsafe');
}
...
function getStore(name){try{return JSON.parse(localStorage.getItem(name)||'{}');}catch(e){return {};}}
function setStore(name,data){try{localStorage.setItem(name,JSON.stringify(data));}catch(e){}}
function reactionCount(reviewId,key){const data=getStore('mm_reactions');return reactionSeed(reviewId,key)+Number(data?.[reviewId]?.[key]||0);}
...
function renderReactions(reviewId){
  ...
  const user=getStore('mm_user_reactions'); const selected=user[reviewId]||'';
...
function handleReaction(e){
  ...
  const data=getStore('mm_reactions'), user=getStore('mm_user_reactions');
  if(user[reviewId]!==key){
    user[reviewId]=key; data[reviewId]=data[reviewId]||{}; data[reviewId][key]=Number(data[reviewId][key]||0)+1;
    setStore('mm_user_reactions',user); setStore('mm_reactions',data);
  }
  renderReactions(reviewId);
  const fresh=document.querySelector(`[data-review="${reviewId}"][data-reaction="${key}"]`);
  if(fresh){pulseEmoji(fresh);spawnParticles(fresh);}
  updateCommunityFavorite();
}
function pulseEmoji(btn){
  const emoji=btn.querySelector('.emoji')?.textContent||'🤘';
  const pop=document.createElement('span'); pop.className='pop-emoji'; pop.textContent=emoji; pop.style.left='16px'; pop.style.top='-10px';
  btn.appendChild(pop); setTimeout(()=>pop.remove(),700);
}
function spawnParticles(btn){
  const rect=btn.getBoundingClientRect();
  for(let i=0;i<7;i++){
    const p=document.createElement('span'); p.className='pop-particle';
    p.style.left=(rect.left+rect.width/2)+'px'; p.style.top=(rect.top+rect.height/2)+'px';
    p.style.setProperty('--x',((Math.random()*80)-40)+'px'); p.style.setProperty('--y',((Math.random()*-70)-12)+'px');
...
window.showReview=showReview; window.showHome=showHome;
...
function updateCommunityFavorite(){
  ...
  section.innerHTML=`<div><span>Community feiert gerade</span><h2>${esc(winner.song)}</h2><p>${esc(winner.artist)} · ${esc(winner.genre)} · ${totalReactionsFor(winner)} Reaktionen</p></div><strong>${top.emoji}</strong>`;
  section.onclick=()=>showReview(winner.id);
}
...
function init(){
  ...
  if(input){
    input.addEventListener('input',()=>{
      ...
      document.querySelectorAll('.review-card-x').forEach(c=>c.style.display=c.dataset.search.includes(q)?'block':'none');
    });
  }
  ...
  if(randomButton){
    randomButton.addEventListener('click',()=>{
      ...
      showReview(reviews[Math.floor(Math.random()*reviews.length)].id);
    });
  }
